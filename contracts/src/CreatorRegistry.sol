...
contract CreatorRegistry {
    ...
    event CreatorRegistered(address indexed creator, string name, string profileURI, uint256 registeredAt);
    ...
    function registerCreator(string calldata name, string calldata profileURI) external {
        ...
        creators[msg.sender] =
            Creator({name: name, profileURI: profileURI, registeredAt: block.timestamp, active: true});

        emit CreatorRegistered(msg.sender, name, profileURI, block.timestamp);
    }
    ...
    function getCreator(address creator)
        external
        view
        returns (string memory name, string memory profileURI, uint256 registeredAt, bool active)
    {
        Creator memory creatorData = creators[creator];

        return (creatorData.name, creatorData.profileURI, creatorData.registeredAt, creatorData.active);
    }
