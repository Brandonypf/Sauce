...
contract RoyaltyManager {
    ...
    event RoyaltyPaid(address indexed creator, uint256 creatorAmount, uint256 platformAmount);
    ...
    function distributeRoyalty(address payable creator) external payable {
        ...
        uint256 creatorAmount = (msg.value * CREATOR_PERCENT) / 100;
        ...
        emit RoyaltyPaid(creator, creatorAmount, platformAmount);
    }

    /**
     * @notice Permite consultar porcentajes.
     */
    function getRoyaltyInfo() external pure returns (uint256 creatorPercent, uint256 platformPercent) {
        return (CREATOR_PERCENT, PLATFORM_PERCENT);
    }
}
