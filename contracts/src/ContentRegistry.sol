...
contract ContentRegistry {
    ...
    event ContentRegistered(uint256 indexed contentId, address indexed creator, string title, uint256 price);
    event ContentUpdated(uint256 indexed contentId, string metadataURI, uint256 price);
    ...
    function registerContent(string calldata title, string calldata metadataURI, bytes32 contentHash, uint256 price)
        external
        returns (uint256)
    {
        require(creatorRegistry.isRegisteredCreator(msg.sender), "Creator not registered");
        ...
        emit ContentRegistered(nextContentId, msg.sender, title, price);
    ...
    function updateContent(uint256 contentId, string calldata metadataURI, uint256 price) external {
        Content storage content = contents[contentId];

        require(content.creator == msg.sender, "Not content owner");

        content.metadataURI = metadataURI;
        content.price = price;

        emit ContentUpdated(contentId, metadataURI, price);
    }
}
