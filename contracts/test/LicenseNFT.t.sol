...
contract LicenseNFTTest is Test {
    ...
    function setUp() public {
        ...
        creatorRegistry.registerCreator("Manga Studio", "ipfs://creator");

        vm.prank(creator);

        contentRegistry.registerContent(
            "Capitulo 1", "ipfs://chapter1", keccak256(abi.encodePacked("chapter-file")), 0.01 ether
        );
    }
    ...
    function testBuyLicense() public {
        ...
        assertEq(licenseNFT.balanceOf(buyer, 1), 1);

        uint256 creatorReceived = creator.balance - creatorBalanceBefore;

        assertEq(creatorReceived, 0.0095 ether);
    }
