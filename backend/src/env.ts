...
const schema = z.object({
  ...
  // Almacenamiento. `local` escribe en disco y sirve para desarrollo; `s3` vale
  // tanto para Cloudflare R2 como para AWS S3 — R2 es compatible con su API.
  STORAGE_DRIVER: z.enum(["local", "s3"]).default("local"),
  STORAGE_LOCAL_DIR: z.string().default("./storage"),
  PUBLIC_API_URL: z.string().default("http://localhost:4000"),

  S3_BUCKET: z.string().default("sauce"),
  S3_REGION: z.string().default("auto"),
  S3_ENDPOINT: z.string().default(""),
  S3_ACCESS_KEY_ID: z.string().optional(),
  S3_SECRET_ACCESS_KEY: z.string().optional(),

  // Tope por archivo. Una novela visual puede pesar varios GB; el límite existe
  // para que nadie pida permiso de subida para 400 GB.
  MAX_UPLOAD_BYTES: z.coerce.number().default(8 * 1024 * 1024 * 1024),
});
