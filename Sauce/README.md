...
2. **Dirección visual.** Sin tocar. `globals.css` no declara ninguna variable CSS,
   mientras `tailwind.config.ts` mapea todos los colores a `--bg-base`,
   `--text-primary`, `--accent-primary` y `--font-inter`. Los componentes están
   renderizando con valores inválidos. Y hay dos estéticas incompatibles
   conviviendo: el vidrio oscuro de `globals.css` frente al sistema claro y cálido
   que implican la configuración de Tailwind y `lightTheme({ accentColor: "#D94F3D" })`.
