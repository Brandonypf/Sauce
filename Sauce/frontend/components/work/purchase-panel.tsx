  ...
  const handlePurchase = async () => {
    ...
    if (!authenticated) {
      try {
        ...
        toast("error", "No se pudo iniciar sesion con tu wallet");
        return;
      }
    }
    ...
    try {
      const checkout = await api.checkout.start(work.slug, idempotencyKey);
      window.location.href = checkout.redirectUrl;
    } catch (error) {
      if (error instanceof ApiError && error.code === "already_owned") {
        markOwned(work.slug);
        toast("success", "Ya tienes esta obra en tu biblioteca");
        return;
      }

      toast("error", error instanceof ApiError ? error.message : "No se pudo iniciar la compra");
    } finally {
      setStarting(false);
    }
  };
    ...
    <div className="space-y-4">
      ...
      {owned ? (
        ...
        <Button
          ...
          loading={starting || signingIn}
        >
          {!starting && !signingIn &&
            (isConnected ? <Play className="h-5 w-5" /> : <Wallet className="h-5 w-5" />)}
          {signingIn
            ? "Firma en tu wallet..."
            : starting
              ? "Abriendo el pago..."
